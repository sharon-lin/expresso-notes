...
// Modified by Stephen Suen for MIT 6.170 Project 2
...
(function() {
  ...
  AbstractChosen = (function() {
    ...
    AbstractChosen.default_multiple_text = "Enter the user's email address";

    AbstractChosen.default_single_text = "Select an Option";

    AbstractChosen.default_no_result_text = "User not found for";
...
(function() {
  ...
  Table = function(selector) {
    var c, cell, cols, data, r, rows, self, table, _i, _j;
    rows = $(selector).find('tbody tr').length;
    cols = $(selector).find('th').length;
    data = [];
    $(selector).find('tbody td a').each(function() {
      return data.push($(this).html());
    });
    table = {
      rows: {
        0: ''
    ...
    for (r = _i = 0; 0 <= rows ? _i < rows : _i > rows; r = 0 <= rows ? ++_i : --_i) {
      table['rows'][r] = data.slice(r * cols, (r + 1) * cols);
    }
    for (c = _j = 0; 0 <= cols ? _j < cols : _j > cols; c = 0 <= cols ? ++_j : --_j) {
      table['cols'][c] = (function() {
        var _k, _len, _ref, _results;
        _ref = data.slice(c);
        _results = [];
        for ((cols > 0 ? (_k = 0, _len = _ref.length) : _k = _ref.length - 1); cols > 0 ? _k < _len : _k >= 0; _k += cols) {
          cell = _ref[_k];
          _results.push(cell);
        }
        return _results;
    ...
    return self = {
      ...
      sort: function(c, d) {
        var date, i, j, new_table, numeric, order, pos, presort, sorted, _k, _l, _len, _m;
        presort = self.col(c);
        sorted = presort.slice(0);
        date = new Date(sorted[0]);
        if (isNaN(date.getMonth())) {
          numeric = true;
          for (_k = 0, _len = sorted.length; _k < _len; _k++) {
            c = sorted[_k];
            numeric = !isNaN(parseInt(c)) && numeric;
          }
          if (numeric) {
          ...
          sorted.sort(function(a, b) {
            ...
            d1 = new Date(a);
            d2 = new Date(b);
            return result = d1 < d2 ? 1 : d1 < d2 ? -1 : 0;
          });
          sorted.reverse();
        }
        if (d === 0) {
        ...
        order = (function() {
          ...
          _results = [];
          for (_l = 0, _len1 = sorted.length; _l < _len1; _l++) {
            pos = sorted[_l];
            _results.push(presort.indexOf(pos));
          }
          return _results;
        })();
        new_table = {
        ...
        for (i = _l = 0; 0 <= rows ? _l < rows : _l > rows; i = 0 <= rows ? ++_l : --_l) {
          new_table['rows'][i] = table['rows'][order[i]];
        }
        for (j = _m = 0; 0 <= cols ? _m < cols : _m > cols; j = 0 <= cols ? ++_m : --_m) {
          new_table['cols'][j] = (function() {
            var _n, _results;
            _results = [];
            for (i = _n = 0; 0 <= rows ? _n < rows : _n > rows; i = 0 <= rows ? ++_n : --_n) {
              _results.push(table['cols'][j][order[i]]);
            }
            return _results;
          })();
        }
  ...
  $('#notes th').click(function() {
    var i, order, rows, _i, _ref, _results;
    rows = $('#notes tbody tr');
    ...
    _results = [];
    for (i = _i = 0, _ref = rows.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      _results.push($('#notes tbody').append(rows[order[i]]));
    }
    return _results;
  });

  $(document).ready(function() {
    return $("#note_user_ids").chosen().css('visibility', 'visible');
  });
